...
const Sell = () => {
    ...
    useEffect(() => {
        if ([name, description, price, currency, address].every(el => el.allValid) && images) {
            setAllValid(true)
        } else {
            setAllValid(false)
        }
                    ...
                    <div className='grid gap-2 pt-5'>
                        ...
                        <div className='grid gap-3 w-fit'>
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setImageRotation(prevRotation => prevRotation - 90)} type='button' className='p-2 text-xl text-accent-orange'><AiOutlineRotateLeft /></button>
                                <input min={1} max={2} step={0.01} onChange={(e) => setImageSize(e.target.value)} className='grow' type="range" />
                                <button onClick={() => setImageRotation(prevRotation => prevRotation + 90)} type='button' className='p-2 text-xl text-accent-orange'><AiOutlineRotateRight /></button>
                            </div>
                            <button onClick={saveImage} className='button-outline-orange' type='button'>Save image</button>
                        </div>
                    </div>
                                ...
                                {images.map((el, key) =>
                                    <div key={key} className='w-full p-2 rounded-lg border-2 grid gap-2'>
                                        ...
                                        <div className="flex gap-2 w-full items-center justify-between">
                                            <button type='button' onClick={() => setMainImageIndex(key)} className={`h-4 w-4 rounded-full ${key === mainImageIndex ? 'bg-accent-orange' : 'border-2'}`}></button>
                                            <button type='button' onClick={() => deleteImage(key)} className='text-accent-red'><BsTrashFill /></button>
                                        </div>
                                    </div>)}
                            </div> : <p className='col-span-2 text-accent-red font-bold'>Upload at least one image to place an item</p>}
                    </div>
                    <button disabled={!allValid} className="button-fill-orange mt-5 disabled:border-2 disabled:bg-white disabled:cursor-not-allowed disabled:border-slate-600 disabled:text-slate-600"><AiOutlineCheck />Place item</button>
                </form> : null}
            {product ?
                <div className='grid gap-5'>
                    ...
                    <button className='button-outline-orange' onClick={() => { setCategory(null); setProduct(null) }}>Sell another item</button>
                </div> : null
            }
        </div >
    )

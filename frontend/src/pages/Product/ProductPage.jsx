...
import { ReadMore, SEO } from '../../components'
...
const ProductPage = () => {
    ...
    return (
        <>
            <SEO
                title={`${data?.name || ''} - Grocket`}
                image={`${window.location.host}${data?.images[0]?.image || ''}`}
                description={`Buy ${
                    data?.name || productId
                } on Grocket for just ${data?.price || ''}${
                    data?.price_currency || ''
                }`}
            />
            {isLoading ? (
                <Spinner type="productPage" />
            ) : (

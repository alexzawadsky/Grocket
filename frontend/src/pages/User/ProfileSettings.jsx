...
export const PasswordReset = () => {
    ...
    return (
        ...
        >
            <BackButton />
            <h2 className="text-2xl font-bold">{t('change_pass')}</h2>
            <Input
                title={t('old_pass')}
                type="password"
                instance={oldPwd}
    ...
    return (
        <div className="grid gap-3">
            ...
            <h2 className="text-2xl font-bold">
                {t('choose_avatar_and_click')} <i>{t('save_adj')}</i>
            </h2>
            <input
                type="file"
                onChange={(e) => setImageInInput(e.target.files[0])}
            />
...
export const DeleteProfile = () => {
    ...
    const handleDelete = () => {
        ...
        api.delete('/api/v1/users/me/', data)
            .then((res) => {
                if (res.status === 204) {
                    logoutUser()
                }
    ...
    return (
        <div className="grid w-full gap-3 md:w-2/3 lg:w-1/2">
            <BackButton />
            <h2 className="text-2xl font-bold">{t('delete_profile')}</h2>
            <h1 className="rounded-xl border-2 border-accent-red p-5 text-center text-2xl font-bold text-accent-red">
                {t('delete_heading')}
            </h1>
            <Input
                title={t('password')}
            ...
            <div>
                <input
                    ...
                    id="check"
                />
                <label className="pl-2" for="check">
                    {t('delete_confirm')}
                </label>
            </div>
            <button
...
const ProfileSettings = () => {
    ...
    return (
        <div className="grid w-full gap-3">
            ...
            <NavLink to="password" className="flex items-center gap-2">
                <MdPassword />
                {t('change_pass')}
            </NavLink>
            <NavLink to="avatar" className="flex items-center gap-2">
                <BsPersonBoundingBox />
                {t('change_avatar')}
            </NavLink>
            <NavLink to="info" className="flex items-center gap-2">
                <BsPersonLinesFill />
                {t('change_profile_info')}
            </NavLink>
            <NavLink
                to="delete"
                className="flex items-center gap-2 font-bold text-accent-red"
            >
                <BsFillPersonXFill />
                {t('delete_profile')}
            </NavLink>
